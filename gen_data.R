...
# generator func
generator <- function(N,P,g){
    # band and sf by flare
    if(g == "band"){
        library('flare')
        G <- sugm.generator(n=N, d=P, graph=g, v=0.3,u=0,g=1)
        theta_0 = G$omega
        sigma_0 = G$sigma
        data = G$data
        S = G$sigmahat
        ...
        library('flare')
        G <- sugm.generator(n=N, d=P, graph="scale-free")
        theta_0 = G$omega
        sigma_0 = G$sigma
        data = G$data
        S = G$sigmahat
...
if(length(args)<=0){
    dist.Gtype <- c("band", "sf", "er", "knn")
    dist.p <- c(50,100,200,400,800)
    # dist.Gtype <- c("er", "knn")
    # dist.p <- c(10)
}else{
    args_list <- paste(unlist(args),collapse=' ')
    args_list <- unlist(strsplit(args_list,'--g'))[-1]
    args_glist <- unlist(strsplit(args_list,'--p'))[1]
    args_mlist <- unlist(strsplit(args_list,'--p'))[-1]
    args.g <- sapply(args_glist, function(x){
        unlist(strsplit(args_glist, ' '))[-1]
        })
    args.p <- sapply(args_mlist, function(x){
        unlist(strsplit(args_mlist, ' '))[-1]
        })
...
for(i in 1:length(dist.Gtype)){
    ...
    for(p_idx in 1:length(dist.p)){
        ...
        G <- generator(N = n, P = P, g = graph)
        save_dir_p = paste0(save_dir, P, ".rds")
